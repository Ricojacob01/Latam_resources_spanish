...
CREATE OR REFRESH MATERIALIZED VIEW ${user_suffix}_gold.resumen_diario_sucursal (
...
FROM ${user_suffix}_silver.transacciones t
LEFT JOIN ${user_suffix}_silver.sucursales s ON t.sucursal_id = s.sucursal_id
GROUP BY ALL;

-- Resumen por producto y canal
CREATE OR REFRESH MATERIALIZED VIEW ${user_suffix}_gold.resumen_producto_canal (
...
FROM ${user_suffix}_silver.transacciones
GROUP BY ALL;

-- Métricas de clientes por segmento
CREATE OR REFRESH MATERIALIZED VIEW ${user_suffix}_gold.metricas_clientes (
...
FROM ${user_suffix}_silver.clientes c
LEFT JOIN ${user_suffix}_silver.cuentas cta ON c.customer_id = cta.cliente_id
LEFT JOIN ${user_suffix}_silver.transacciones t ON c.customer_id = t.cliente_id
GROUP BY c.segmento;
