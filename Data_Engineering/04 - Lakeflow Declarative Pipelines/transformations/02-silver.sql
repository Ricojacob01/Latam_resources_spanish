...
CREATE OR REFRESH STREAMING TABLE ${user_suffix}_silver.transacciones (
...
AS SELECT
  ...
  sucursal_id,
  producto
FROM STREAM(${user_suffix}_bronze.transacciones_raw);

-- Cuentas limpias
CREATE OR REFRESH STREAMING TABLE ${user_suffix}_silver.cuentas (
...
AS SELECT
  ...
  estado
FROM STREAM(${user_suffix}_bronze.cuentas_raw);

-- Sucursales
CREATE OR REFRESH STREAMING TABLE ${user_suffix}_silver.sucursales  (
...
AS SELECT
  ...
  provincia,
  region
FROM STREAM(${user_suffix}_bronze.sucursales_raw);

-- AUTO CDC: Clientes (SCD Tipo 2)
CREATE OR REFRESH STREAMING TABLE ${user_suffix}_silver.clientes (
...
CREATE FLOW clientes_cdc_flow AS AUTO CDC INTO ${user_suffix}_silver.clientes
FROM STREAM(${user_suffix}_bronze.clientes_cdc_raw)
KEYS (customer_id)
