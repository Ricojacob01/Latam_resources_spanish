...
-- MAGIC
-- MAGIC 1. En la **barra lateral izquierda**, haz clic en el botón **+ New** (arriba)
-- MAGIC 2. Selecciona **ETL Pipeline**
-- MAGIC
-- MAGIC > **Importante (UI nueva 2026)**: al hacer click en *ETL Pipeline*, Databricks
-- MAGIC > abre directamente el **Lakeflow Pipelines Editor** con un archivo
-- MAGIC > `my_transformation` ya creado. No hay un modal intermedio — la configuración
-- MAGIC > (nombre, catálogo, schema) se ajusta desde el toolbar arriba del editor.
-- MAGIC
-- MAGIC ### Paso 2: Renombrar y configurar el pipeline desde el toolbar
-- MAGIC
-- MAGIC En la barra superior del editor verás (de izquierda a derecha):
-- MAGIC `[Nombre del pipeline]` · `Catalog and schema` · `Language`
-- MAGIC
-- MAGIC 1. **Nombre del pipeline** (click sobre el nombre por defecto para editarlo):
-- MAGIC    - `Colsubsidio Workshop - [tu-usuario]`
-- MAGIC    - Ejemplo: `Colsubsidio Workshop - john_doe`
-- MAGIC
-- MAGIC 2. **Catalog and schema** (click a la derecha del nombre):
-- MAGIC    - **Catalog**: `colsubsidio_workshop`
-- MAGIC    - **Schema**: `sdp_workshop_<tu_usuario>_bronze`
-- MAGIC    - Estos son los valores **por defecto** — cualquier tabla creada sin
-- MAGIC      `catalog.schema.` explícito vivirá ahí.
-- MAGIC
-- MAGIC ### Paso 3: Conectar la carpeta `transformations` del workshop
-- MAGIC
-- MAGIC La carpeta `transformations/` del workshop ya contiene `orders_pipeline.sql`.
-- MAGIC En el panel izquierdo (asset browser):
-- MAGIC
-- MAGIC 1. Click en **⚙️ Settings** del toolbar
-- MAGIC 2. En la pestaña **Pipeline assets** → **Code assets**:
-- MAGIC    - Bajo **Root folder**, click **Edit** y selecciona la carpeta `colsubsidio` que importaste
-- MAGIC    - En **Source code paths**, agrega `transformations`
-- MAGIC 3. **Save**
-- MAGIC
-- MAGIC El editor ahora reconoce `orders_pipeline.sql` como código del pipeline.
-- MAGIC
...
-- MAGIC 5. Repetir el processo y agrega las  **configuration** para los variables de bronze,silver, y gold 
-- MAGIC    - **Key**: `bronze`
-- MAGIC    - **Value**: 
-- MAGIC    Formato: `sdp_workshop_{username}_bronze`
-- MAGIC    -Ejemplo: `sdp_workshop_john_doe_bronze`
-- MAGIC    - **Key**: `silver`
-- MAGIC    - **Value**: 
-- MAGIC    Formato: `sdp_workshop_{username}_silver`
-- MAGIC    -Ejemplo: `sdp_workshop_john_doe_silver`
-- MAGIC     - **Key**: `gold`
-- MAGIC    - **Value**: 
-- MAGIC    Formato: `sdp_workshop_{username}_gold`
-- MAGIC    -Ejemplo: `sdp_workshop_john_doe_gold`
...
-- MAGIC
-- MAGIC working_dir = f'/Volumes/{catalog_name}/sdp_workshop_{clean_username}_default/raw'
-- MAGIC
...
-- MAGIC %python
-- MAGIC # Create widget with the catalog name
-- MAGIC dbutils.widgets.text("bronze", f"sdp_workshop_{clean_username}_bronze")
-- MAGIC dbutils.widgets.text("silver", f"sdp_workshop_{clean_username}_silver")
-- MAGIC dbutils.widgets.text("gold", f"sdp_workshop_{clean_username}_gold")
