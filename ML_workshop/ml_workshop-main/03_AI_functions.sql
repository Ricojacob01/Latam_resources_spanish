...
-- PUNTO ÚNICO DE EDICIÓN: sustituye `latam_hunter` por tu catálogo y `rico` por tu nombre.
-- El resto del notebook usa el catálogo/esquema activo gracias a los USE de abajo.
CREATE SCHEMA IF NOT EXISTS `latam_hunter`.`demo_ai_functions_rico`;

CREATE VOLUME IF NOT EXISTS `latam_hunter`.`demo_ai_functions_rico`.`archivos`;

USE CATALOG `latam_hunter`;
USE SCHEMA `demo_ai_functions_rico`;

-- COMMAND ----------

-- MAGIC %python
-- MAGIC
-- MAGIC catalog = "latam_hunter"            # mismo catálogo que arriba
-- MAGIC schema = "demo_ai_functions_rico"   # mismo esquema que arriba
