...
-- DBTITLE 1,Granting access to Data Scientists & Data Engineers:
-- MAGIC %python
-- MAGIC # Otorgamos permisos a grupos que EXISTEN en este workspace.
-- MAGIC # Revisa Admin Console > Groups y ajusta los nombres si tu cuenta usa otros.
-- MAGIC # Usamos las variables `catalog` y `db` definidas en ./_resources/00-setup,
-- MAGIC # de modo que esto apunta siempre a TU esquema (no a uno fijo).
-- MAGIC spark.sql("GRANT SELECT ON TABLE mlops_churn_bronze_customers TO `data scientist`")
-- MAGIC spark.sql(f"GRANT SELECT, MODIFY ON SCHEMA `{catalog}`.`{db}` TO `data engineers`")
...
-- MAGIC Nuestro primer trabajo es analizar los datos y preparar un conjunto de características.
-- MAGIC
-- MAGIC Siguiente: [Analizar los datos y preparar características]($./02_1_feature_engineering)
