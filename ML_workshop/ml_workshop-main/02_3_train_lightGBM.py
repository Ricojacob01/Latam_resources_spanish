...
# SPARK_LOCAL_DIRS exists on classic clusters; on Serverless it may not be set,
# so we fall back to the system temp dir. Works on both.
eval_temp_dir = os.path.join(os.environ.get("SPARK_LOCAL_DIRS", tempfile.gettempdir()), "tmp", str(uuid.uuid4())[:8])
