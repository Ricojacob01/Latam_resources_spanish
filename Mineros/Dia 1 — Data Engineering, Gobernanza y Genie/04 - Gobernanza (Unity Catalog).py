...
# DBTITLE 1,Introducción a gobernanza
# MAGIC %md
...
# MAGIC         <img src="https://github.com/databricks-demos/dbdemos-resources/raw/main/images/alice.png" width="60px"> <br/>
# MAGIC         Analistas de Datos<br/>
# MAGIC         <img src="https://github.com/databricks-demos/dbdemos-resources/raw/main/images/marc.png" width="60px"> <br/>
# MAGIC         Científicos de Datos<br/>
# MAGIC         <img src="https://github.com/databricks-demos/dbdemos-resources/raw/main/images/john.png" width="60px"> <br/>
...
# DBTITLE 1,Requisito de cómputo
# MAGIC %md
...
# DBTITLE 1,Imagen serverless
# MAGIC %md
# MAGIC ![](./files/images/version2-serverless.png)

# COMMAND ----------

# DBTITLE 1,Unity Catalog overview
# MAGIC %md
...
# DBTITLE 1,Explorar base de datos
# MAGIC %md
...
# DBTITLE 1,Configurar catálogo y esquema
# Catálogo compartido + tu esquema (mismo que en el Setup)
import re
...
# DBTITLE 1,Consultar catálogo actual
# MAGIC %sql
# MAGIC SELECT CURRENT_CATALOG();

# COMMAND ----------

# DBTITLE 1,Consultar esquema actual
# MAGIC %sql
# MAGIC SELECT CURRENT_SCHEMA();

# COMMAND ----------

# DBTITLE 1,Crear tabla bronze customers
...
# DBTITLE 1,Revisar tablas creadas
# MAGIC %md
...
# DBTITLE 1,Mostrar tablas del esquema
# MAGIC %sql
# MAGIC SHOW TABLES

# COMMAND ----------

# DBTITLE 1,Otorgar permisos básicos
# MAGIC %sql
# MAGIC -- Vamos a otorgar a nuestros ANALISTAS el permiso de SELECT:
# MAGIC -- Nota: asegúrate de haber creado previamente los grupos analysts y dataengineers desde la consola de la cuenta.
# MAGIC GRANT SELECT ON TABLE bronze_customers TO `analysts`;
# MAGIC
# MAGIC -- Otorgaremos un permiso adicional de MODIFY a nuestro Ingeniero de Datos
# MAGIC GRANT SELECT, MODIFY ON SCHEMA bronze TO `dataengineers`;

# COMMAND ----------

# DBTITLE 1,Introducción a PII
# MAGIC %md
...
# DBTITLE 1,Crear tabla protegida
# MAGIC %sql
# MAGIC CREATE OR REPLACE TABLE churn_protected AS
# MAGIC SELECT
# MAGIC   *,
# MAGIC   "exemplo@email.com" AS email,
# MAGIC   "Jorge"             AS firstname,
# MAGIC   "Vidal"             AS lastname,
# MAGIC   "Calle 123"         AS address,
# MAGIC   CASE
# MAGIC     WHEN ROW_NUMBER() OVER (ORDER BY customer_id) <= (COUNT(*) OVER () / 2) THEN 'Chile' 
# MAGIC     ELSE 'Brazil'
...
# DBTITLE 1,Consultar tabla protegida
# MAGIC %sql
# MAGIC SELECT * FROM churn_protected;

# COMMAND ----------

# DBTITLE 1,Consultar países protegidos
# MAGIC %sql
# MAGIC SELECT DISTINCT country
# MAGIC FROM churn_protected;

# COMMAND ----------

# DBTITLE 1,Aplicar masks y row filter
# MAGIC %sql
# MAGIC -- El grupo de retail_admin va a tener acceso a todos los datos, el resto de usuarios va a ver la información enmascarada 
# MAGIC CREATE OR REPLACE FUNCTION simple_mask(column_value STRING)
# MAGIC    RETURN IF(is_account_group_member('retail_admin'), column_value, "****");
# MAGIC    
# MAGIC -- Enmascara toda la información PII
# MAGIC ALTER TABLE churn_protected ALTER COLUMN email SET MASK simple_mask;
# MAGIC ALTER TABLE churn_protected ALTER COLUMN firstname SET MASK simple_mask;
# MAGIC ALTER TABLE churn_protected ALTER COLUMN lastname SET MASK simple_mask;
# MAGIC ALTER TABLE churn_protected ALTER COLUMN address SET MASK simple_mask;
# MAGIC
# MAGIC -- Aplica un filtro de filas para la columna país 
# MAGIC CREATE OR REPLACE FUNCTION country_filter(country STRING) 
# MAGIC RETURN (
# MAGIC   is_account_group_member('retail_admin') or
# MAGIC   country like "Chile"
# MAGIC );                 
# MAGIC
# MAGIC ALTER TABLE churn_protected SET ROW FILTER country_filter ON (country);
# MAGIC
# MAGIC SELECT * FROM churn_protected

# COMMAND ----------

# DBTITLE 1,Validar países visibles
# MAGIC %sql
# MAGIC SELECT DISTINCT country
# MAGIC FROM churn_protected;
...
# DBTITLE 1,Cierre de gobernanza
# MAGIC %md
