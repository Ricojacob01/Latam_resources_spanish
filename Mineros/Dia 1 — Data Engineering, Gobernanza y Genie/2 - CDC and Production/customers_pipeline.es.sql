...
--
-- MODELO DE NOMBRES: catálogo por defecto = academia, esquema por defecto = <tu_apellido>.
-- Las tablas se nombran SIN esquema y usan sufijos de capa (_raw/_clean/_silver/_gold).
-------------------------------------------------------
...
AUTO CDC INTO customers_silver                 -- Tabla destino a mantener
FROM STREAM customers_clean                    -- Eventos CDC origen
...
CREATE OR REFRESH MATERIALIZED VIEW customer_summary_gold
...
--
-- CREATE OR REFRESH STREAMING TABLE customers_history_silver
--   COMMENT "Histórico de clientes (SCD Tipo 2)";
--
-- CREATE FLOW customers_cdc_flow_scd2 AS
-- AUTO CDC INTO customers_history_silver
-- FROM STREAM customers_clean
--   KEYS (customer_id)
